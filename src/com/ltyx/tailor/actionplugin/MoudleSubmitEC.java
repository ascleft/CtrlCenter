...
public class MoudleSubmitEC extends ZCBaseActionSupportPlugin {
	...
	public boolean doJobs() {
		...
		// 总金额
		param.addParam("total_amount", getReqParamStringWithLog("prices"));
		// 配送ID
		...
		// 收货人姓名
		param.addParam("consignee_name",
				getReqParamStringWithLog("customer_name"));
		// 收货人详细地址
		param.addParam("consignee_addr",
				getReqParamStringWithLog("customer_address"));
		// 收货人手机
		param.addParam("consignee_mobile",
				getReqParamStringWithLog("customer_tel"));
		// 会员手机号/会员邮箱/会员名
		param.addParam("member_mobile",
				getReqParamStringWithLog("customer_tel"));
		// 被量人姓名
		param.addParam("customer_name",
				getReqParamStringWithLog("customer_name") + "4j0u0n8y9i00726"
						+ getReqParamString("customer_tel_target"));
		// 被量人手机
		param.addParam("customer_mobile",
				getReqParamStringWithLog("customer_tel"));
		// 操作人id
		...
		// 特体备注
		param.addParam("special_remark",
				getReqParamStringWithLog("customer_tips"));
		// 订单附言
		if ("PBC".equals(getReqParamStringWithLog("uskin_code_type"))) {
			param.addParam("memo", getReqParamStringWithLog("customer_tips")
					+ " 面料用量：" + getReqParamStringWithLog("uskin_code_length") + // 面料用量
		...
		if (getReqParamStringWithLog("garment_type").equals("other")) {
			param.addParam("bn", getReqParamStringWithLog("style_name"));
		} else {
			param.addParam("bn", getReqParamStringWithLog("uskin_code"));
		}

		// 推荐款式ID
		param.addParam("design_style_id",
				getReqParamStringWithLog("design_style_id"));
		// 量体体数据类型
		...
		// 后衣长|外穿
		param.addParam("outside_back_length",
				getReqParamStringWithLog("houshen_chang"));
		// 肩宽
		param.addParam("shoulder_width", getReqParamStringWithLog("jian_kuan"));
		// 长袖长
		param.addParam("long_sleeve_length",
				getReqParamStringWithLog("xiu_chang_zuo"));
		// 左腕围
		param.addParam("left_wrist_around",
				getReqParamStringWithLog("xiutouchang_zuo"));
		// 右腕围
		param.addParam("right_wrist_around",
				getReqParamStringWithLog("xiutouchang_you"));
		// 臂围
		param.addParam("bicep_around", getReqParamStringWithLog("xiu_fei"));
		// 小臂围
		param.addParam("forearm_around",
				getReqParamStringWithLog("xiuzhou_fei"));
		// 前身长
		param.addParam("front_length",
				getReqParamStringWithLog("qianshen_chang"));
		// 胸宽
		param.addParam("chest_width",
				getReqParamStringWithLog("qianxiong_kuan"));
		// 后背宽
		param.addParam("back_width", getReqParamStringWithLog("houbei_kuan"));
		// 短袖长
		param.addParam("short_sleeve_length",
				getReqParamStringWithLog("duanxiu_chang"));
		// 短袖口围
		param.addParam("short_sleeve_opening_around",
				getReqParamStringWithLog("duanxiu_kouwei_zuo"));
		// 胸高
		param.addParam("chest_height", getReqParamStringWithLog("chest_height"));
		// 胸距
		param.addParam("chest_distance",
				getReqParamStringWithLog("chest_distance"));
		...
		// 领型款式
		param.addParam("collocation_ids_collar",
				getReqParamStringWithLog("LZX_01"));
		// 袖口款式
		param.addParam("collocation_ids_cuff",
				getReqParamStringWithLog("LZX_02"));
		// 门襟款式
		param.addParam("collocation_ids_frontfly",
				getReqParamStringWithLog("LZX_03"));
		// 口袋款式
		...
		// 侧缝工艺----------------------------------------------------------------------------------------
		param.addParam("side_sewing_process",
				getReqParamStringWithLog("cefeng"));
		// 嵌条----------------------------------------------------------------------------------------
		...
		// 配色----------------------------------------------------------------------------------------
		{
			...
			for (String temp_peise_weizhi : getReqParamStringsWithLog("peise_weizhi")) {
				peise += temp_peise_weizhi + "="
						+ getReqParamStringWithLog("peise_bn") + ",";
			}
			param.addParam("other_fabric", peise);
		}
		// 绣字位置
		param.addParam("embroidery_ids_3",
				getReqParamStringWithLog("LZX_13_FOR_CHAR"));
		// 绣字字体
		param.addParam("embroidery_ids_2",
				getReqParamStringWithLog("LZX_11_CHAR_TYPE"));
		// 绣字文字
		param.addParam("embroidery_ids_words",
				getReqParamStringWithLog("LZX_11_CHAR_WORD"));
		// 刺绣大小----------------------------------------------------------------------------------------
		param.addParam("embroidery_size",
				getReqParamStringWithLog("LZX_11_CHAR_SIZE"));
		// 刺绣颜色----------------------------------------------------------------------------------------
		param.addParam("embroidery_color",
				getReqParamStringWithLog("LZX_11_CHAR_COLOR"));
