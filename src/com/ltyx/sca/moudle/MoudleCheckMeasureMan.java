...
public class MoudleCheckMeasureMan extends ZCBaseActionSupportPlugin {
	...
	private boolean check_maleMeasure() {
		for (String key : maleBodyMeasure.keySet()) {
			if (!maleBodyMeasure.get(key).check()) {
	...
	private void init() {
		...
		maleBodyMeasure.put("xiong_wei",           new Cell(true,  "xiong_wei", "胸围", 70, 170));//20181020已经确认
		//maleBodyMeasure.put("yao_wei",           new Cell(true,  "yao_wei", "腰围(中腰围)", 67, 163));//20181020已经确认
	...
	class Cell {
		...
		Cell(boolean isNecessary, String key, String name, double Min, double Max) {
		...
		boolean check() {
			String input_string = getReqParamString(key);
			double input_double = getReqParamDouble(key);
			...
			if (isNecessary) {
				if (input_string.trim().length() == 0) {
					...
					if (input_double < Min) {
						ERRCODE = "0";
						ERRDESC = "fail";
						data = "当前" + name + "的数据为" + StringHelper.numWithLength(input_double, 0, 2) + "厘米," + "已经低于下限" + Min + "厘米," + "请重新填写";
						return false;
					}
					if (input_double > Max) {
						ERRCODE = "0";
						ERRDESC = "fail";
						data = "当前" + name + "的数据为" + StringHelper.numWithLength(input_double, 0, 2) + "厘米," + "已经超过上限" + Max + "厘米," + "请重新填写";
						return false;
					}
					return true;
				}
