...
public class MoudleYBR_OBO_CheckMeasure extends ZCBaseActionSupportPlugin {

	private Map<String, Cell> maleBodyMeasure = null;

	public MoudleYBR_OBO_CheckMeasure(HttpServletRequest req) {
		this.name = "对外接口 衣邦人 校验 尺寸";
		this.request = req;
	}
	...
	private void init() {
		...
		maleBodyMeasure.put("ling_wei",       new Cell(true, "领围",           0, 200));// 20180614耍赖改的
		maleBodyMeasure.put("xiong_wei",      new Cell(true, "胸围",           0, 200));
		maleBodyMeasure.put("yao_wei",        new Cell(true, "腰围(中腰围)",     0, 200));
		maleBodyMeasure.put("dibian",         new Cell(true, "底边(臀围)",      0, 200));
		maleBodyMeasure.put("xiu_fei",        new Cell(true, "袖肥(大臂围)",     0, 200));// 20180614耍赖改的
		maleBodyMeasure.put("jian_kuan",      new Cell(true, "肩宽",           0, 200));
		
		maleBodyMeasure.put("qianshen_chang", new Cell(false, "前身长",         0, 200));// 20180614耍赖改的 20180615傻逼又特么改了一次
